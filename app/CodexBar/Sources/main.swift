...
// Phase 1: native display layer (replaces the SwiftBar plugin). Reads state.json, shows the active
// account's 5h remaining in the menu bar + a per-account dropdown. Quota math mirrors the plugin's
// captured_at-aware win_remaining (a window past its reset is only "100%" when our snapshot predates
// the reset; otherwise used_percent is real). Interaction (switch / refresh-all / notifications) and
// the accuracy daemon land in later phases. Built with plain swiftc (no Xcode) → CodexBar.app.
import AppKit
import Foundation
...
// remaining % for one window; captured_at-aware (see header)
...
final class Controller: NSObject {
    ...
    func rebuild() {
        ...
        // ---- dropdown ----
        let menu = NSMenu()
        let header = NSMenuItem(); header.attributedTitle = mono("CODEX · 剩余额度 · CodexBar 0.1.0", .secondaryLabelColor, 11)
        menu.addItem(header)
        menu.addItem(.separator())

        // sort by label so order is stable (main, plus2, plus3 …)
        ...
        for (aid, slot) in ordered {
            ...
            let dot = dead ? "⚠️" : (aid == active ? "●" : "○")
            let snap = (aid != active && q != nil) ? "  · 快照 \(fmtAge(q?["captured_at"]))" : ""
            let head = NSMenuItem(); head.attributedTitle = mono("\(dot) \(label)\(snap)", nil, 13)
            menu.addItem(head)
            let sub = NSMenuItem(); sub.attributedTitle = mono("   \(email)", .secondaryLabelColor, 11)
            menu.addItem(sub)
            if let q = q {
                let cap = q["captured_at"] as? Double ?? 0
        ...
        let refresh = NSMenuItem(title: "🔄 立即刷新全池额度（各号 +1% 5h）", action: #selector(refreshAll), keyEquivalent: "")
        refresh.target = self
        menu.addItem(refresh)
        let reload = NSMenuItem(title: "重新读取", action: #selector(reload), keyEquivalent: "r")
        reload.target = self
        menu.addItem(reload)
        let quit = NSMenuItem(title: "退出 CodexBar", action: #selector(quitApp), keyEquivalent: "q")
        quit.target = self
        menu.addItem(quit)
        item.menu = menu
    }
    ...
    @objc func refreshAll() {
        ...
        try? p.run()  // async; rebuild() on the next timer tick will pick up fresh state
    }
