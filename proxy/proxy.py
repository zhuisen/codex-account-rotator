...
def _pick(prev_id):
    """(aid, slot, reason). Affinity: stick to prev_id's account; else least-used non-cooling."""
    s = _load(STATE)
    slots = s.get("slots", {})
    now = time.time()
    if not slots:
    ...
    with _lock:
        if prev_id and prev_id in _affinity:
            ...
            if aid in slots and slots[aid].get("cooling_until", 0) <= now:
                return aid, slots[aid], "affinity"
    avail = [(aid, sl) for aid, sl in slots.items() if sl.get("cooling_until", 0) <= now]
    if not avail:
        avail = list(slots.items())  # all cooling → least-bad
    ...
    def _proxy(self):
        ...
        if body:
            try:
                ...
                pass
        aid, slot, reason = _pick(prev_id)
        if not aid:
            self.send_error(503, "no accounts in pool")
            return
        ...
        try:
            ...
            if resp.status == 429:
                _cool(aid)
                sys.stderr.write(f"[proxy] 429 → cooled [{label}] 300m\n")
            ...
            sys.stderr.write(f"[proxy] upstream err: {e}\n")
            try:
