...
def main():
    ...
    # one-tap: probe every account for live quota now (codex-rotate refresh-all --notify). ~2s; each
    # account spends ~1% of its 5h window. The dropdown closes on tap and the menu-bar title (5h, usually
    # ~99%) won't visibly move, so --notify posts a macOS notification as the ACK; refresh-all also calls
    # back into SwiftBar to re-render, and reopening shows fresh "快照 刚刚".
    print(f"🔄 立即刷新全池额度 · 各号 +1% 5h | shell={ROT} param1=refresh-all param2=--notify terminal=false refresh=true")
    print("---")
    if not slots:
