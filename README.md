...
> 日常:用 **`cxp`** 代替 `codex`,你的 Codex 会话就会在多个号之间**逐请求透明轮换**:撞限自动换号、token 自动刷新、长会话消耗摊到所有号 ≈ **把 5 小时上限扩成 N 倍**。菜单栏实时看每号余量。
...
| `swiftbar/codex-status.10s.py` | **SwiftBar 菜单栏插件**(版本号在标题行 `CODEX · 剩余额度 · vX`) |
...
# 3) 看池子 / 手动操作
codex-rotate list                    # 每号谁在用/冷却 + 5h/周用量
codex-rotate switch plus2            # 手动切号(单号模式)
codex-rotate refresh all             # 手动 OAuth 刷新非活跃号
```

菜单栏(SwiftBar)装好后顶部显示当前号 5h 余量,下拉看每号油表 + `🔀 代理轮换 开/关` + 版本号。
