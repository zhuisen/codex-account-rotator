...
#: 缓存里最多保留多少个文件条目。
#:
#: ★ 这个上限是 2026-09-12 跟着「合并而不是替换」一起加的 —— 在那之前，**窄窗口会顺手
#:   把宽窗口需要的条目剪掉**，那种剪裁恰好也起到了"上限"的作用（只是代价是缓存命中率）。
#:   改成合并之后，条目数就等于"这台机器上出现过的全部文件"，必须显式封顶。
#: ★ 20000 的来历：本机实测并集 **12,037 条 / 22.4MB**（90 档 7,489 条，1095 档 12,037 条）。
#:   留 1.7 倍余量，够吸收会话文件的自然增长，又不会让缓存长到读它本身成为热路径的开销。
CACHE_MAX_FILES = 20000


def _merge_cache(cached, fresh):
    """把这一轮解析出来的条目**并回**旧缓存 -> (合并结果, 是否需要落盘)。

    ## 这修的是什么

    ★★★ 原来是 `_save_cache(fresh)` —— 而 `fresh` **只装本次窗口访问过的文件**，
      整份替换等于把窗口之外的条目全删掉。后果是**窄窗口污染宽窗口**：
      跑一次 `--days 90` 会把更早的条目剪光，紧接着的 `--days 365` 只能从头再解析一遍。

      实测（2026-09-12，交替跑）：

          连续 365 / 365 / 365   →  1.52s · 1.04s · 1.04s
          交替 90 → 365 ×3       →  365 每次都 **5.2 ~ 8.3s**，而 90 恒 1.1s

      **不对称正是证据**：宽窗口从不拖慢窄窗口，只有反过来。
      而「AI用量」页正是让用户在 30d 和年度之间来回切 —— 这条缺陷每切一次收 5 秒。
      用户 2026-09-12 的原话：「不要每次选择都是要重新刷新加载，这样会影响使用」。

    ## 为什么不加时间戳

    ★ 用**插入顺序**做 LRU，不给条目加 `seen` 字段：加了的话每轮都要重写时间戳 ⇒
      payload 每轮都变 ⇒ **每轮都要落盘 22MB**，而"无变更就不写缓存"这条正是热路径上
      最大的单项开销（曾是 8.6MB 白写）。这里 `未命中的旧条目` 在前、`本轮的新条目` 在后，
      超限时从**前面**砍，语义就是"最久没被碰过的先走"。
    """
    out = {k: v for k, v in cached.items() if k not in fresh}
    out.update(fresh)
    if len(out) > CACHE_MAX_FILES:
        for k in list(out)[:len(out) - CACHE_MAX_FILES]:
            del out[k]
    # ★ 判据是**键集合**变没变，不是内容 —— 内容变了必然伴随 `scanned`/`incr`，
    #   调用方已经在看那两个。拿内容比对要序列化整份 22MB，比直接写还贵。
    return out, set(out) != set(cached)
...
def scan(days=90, use_cache=True, only=None, exclude=None):
    ...
    # ★★ **合并，不是替换**（2026-09-12，见 `_merge_cache`）。原来这里写的是 `fresh`，
    #    于是窄窗口会把宽窗口的条目剪掉，用户在 30d / 年度之间来回切时每次都要重扫 5 秒。
    if use_cache:
        merged, key_change = _merge_cache(cached, fresh)
        if scanned or incr or key_change:
            _save_cache(merged)
