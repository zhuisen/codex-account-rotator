...
## 1.5 最省事:用 App 里的 Connector 接入（v1.8.0 起）

装完桌面应用（[§5](#5-构建桌面应用)，或直接下载 Releases 里的安装包）之后：

> **CodexBar → 设置 → 「账号池接入（Connector）」** → 勾选 → 点「接入」。

它替你做的正是下面 §2~§4 那几步，逐项可选、每一步都能**先看将要写入的原文**，
并且随时可以「撤销接入」。

| 保证 | 说明 |
|---|---|
| **不需要管理员密码** | 全部落在用户级：`~/Library/LaunchAgents`、`~/.local/bin`、`~/.codex` |
| **不重写你的配置** | 只在托管标记之间写 `~/.codex/config.toml`，标记之外**一个字节都不动** |
| **不碰凭证** | ★ **绝不调用 `codex login` / `codex logout`** —— 那两条会在服务端吊销当前账号 |
| **可撤销** | 「撤销接入」只删它自己建的东西；你自己放的同名文件会先备份再让路 |
| **认得出你手工装过的** | 已存在的 provider / profile / 入口标成「你自己装的」，**不会去覆盖** |

⚠️ 「让 `codex` 本身也走轮换」是**单独一项、默认不勾** —— 它会改变裸 `codex` 的行为。
不勾也能用，日常敲 `cxp` 即可。

**想自己动手、或想完全控制每一步，继续往下看 §2~§4。两条路等价。**
